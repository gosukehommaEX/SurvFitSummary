...
#' Q-Q plot. For 3+ arms, it creates multiple Q-Q plots comparing each treatment
#' arm against the control arm, arranged horizontally.
#'
...
#'     \item Horizontal panel of Q-Q plots if ARM has 3+ levels, each comparing
...
#'   \item Extracts survival quantiles at percentiles: 0.1, 0.2, ..., 0.9
...
#' @importFrom ggplot2 ggplot aes geom_point geom_abline geom_smooth geom_segment labs theme_minimal
#'   theme element_text scale_shape_manual scale_linetype_manual guide_legend guides override.aes
...
#' )
#'
#' # Multiple Q-Q plots (Treatment1 vs Control, Treatment2 vs Control)
#' p2 <- plot_qq_survival(
...
#' )
#'
#' # Three Q-Q plots (TrtA vs Control, TrtB vs Control, TrtC vs Control)
#' p3 <- plot_qq_survival(
...
plot_qq_survival <- function(dataset,
  ...
  for (trt_arm in treatment_arms) {
    ...
    # Create Q-Q plot
    p <- ggplot2::ggplot(qq_data, ggplot2::aes(x = .data[[control]], y = .data[[trt_arm]])) +
      ggplot2::geom_abline(
        intercept = 0,
        slope = 1,
        color = 'black',
  ...
  if (length(plot_list) > 1) {
    ...
    combined_plot <- plot_list[[1]] +
      patchwork::plot_annotation(
        title = 'Q-Q Plot of Survival Percentiles',
        subtitle = 'Comparing survival time distributions between treatment arms',
        theme = ggplot2::theme(
          plot.title = ggplot2::element_text(
            size = 20,
            face = 'bold',
